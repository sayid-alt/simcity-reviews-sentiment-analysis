...
from sklearn.model_selection import LearningCurveDisplay, learning_curve, GridSearchCV
...
def analysis_label(content, lexicon_pos, lexicon_neg):
...
################## FEATURE EXTRACTION ####################
def tfidf_vectorizer(X, fit_on_data=True):
...
def X_y_tokenizing(X, y, vocab_size=10000):
    ''' TOKENIZING FOR FED INTO LSTM '''
    # initiate tokenizer object
    tokenizer = Tokenizer(num_words=vocab_size, oov_token='<OOV>')
    tokenizer.fit_on_texts(X) # fit tokenize on the X
    X_sequences = tokenizer.texts_to_sequences(X) # transform sentences into token sequences
    ...
    return X_padded, labels, tokenizer


def embedding_matrix(tokenizer, word2vec):
...
def tfds_split(X, y):
    ...
    # shuffle the dataset
    train_set = train_set.shuffle(buffer_size=len(train_set)).batch(batch_size=batch_size)
    test_set = test_set.shuffle(buffer_size=len(test_set)).batch(batch_size=batch_size)
